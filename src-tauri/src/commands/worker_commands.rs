...
pub fn stop_worker(name: String) -> CommandResult<WorkerStatus> {
    let table = workers();
    let mut table = table.lock().map_err(|_| command_error("worker table lock failed"))?;
    if let Some(mut child) = table.remove(&name) {
        let _ = child.kill();
        let _ = child.wait();
        return Ok(WorkerStatus { name, pid: None, state: "Stopped".to_string(), binary_path: None, last_error: None });
    }
    Ok(WorkerStatus { name, pid: None, state: "WorkerUnavailable".to_string(), binary_path: None, last_error: Some("Worker was not running".to_string()) })
}
...
pub fn get_worker_status(name: String) -> CommandResult<WorkerStatus> {
    ...
    if let Some(child) = table.get_mut(&name) {
        ...
        return Ok(WorkerStatus { name: name.clone(), pid: Some(pid), state, binary_path: find_worker_binary(&name).map(|p| p.to_string_lossy().to_string()), last_error: None });
    }
    Ok(WorkerStatus { name: name.clone(), pid: None, state: "Stopped".to_string(), binary_path: find_worker_binary(&name).map(|p| p.to_string_lossy().to_string()), last_error: None })
}
...
fn start_worker_inner(request: WorkerStartRequest) -> anyhow::Result<WorkerStatus> {
    let binary = find_worker_binary(&request.name).ok_or_else(|| anyhow::anyhow!("WorkerUnavailable: {}", request.name))?;
    ...
    let pid = child.id();
    workers().lock().map_err(|_| anyhow::anyhow!("worker table lock failed"))?.insert(request.name.clone(), child);
    Ok(WorkerStatus {
        name: request.name,
        pid: Some(pid),
...
fn run_health_inner(name: String) -> anyhow::Result<WorkerHealthOutput> {
    let binary = find_worker_binary(&name).ok_or_else(|| anyhow::anyhow!("WorkerUnavailable: {}", name))?;
...
fn find_worker_binary(name: &str) -> Option<PathBuf> {
    let exe_name = if name.ends_with(".exe") { name.to_string() } else { format!("{name}.exe") };
